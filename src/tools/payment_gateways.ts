...
export const paymentGatewayTools: Tool[] = [
  {
    ...
    inputSchema: zodToJsonSchema(listPaymentGatewaysSchema) as {
      type: "object";
  ...
  },
  {
    ...
    inputSchema: zodToJsonSchema(getPaymentGatewaySchema) as {
      type: "object";
      properties: Record<string, unknown>;
    },
  },
  {
    ...
    inputSchema: zodToJsonSchema(updatePaymentGatewaySchema) as {
      type: "object";
      properties: Record<string, unknown>;
    },
  },
  ...
  list_payment_gateways: async () => {
    try {
      ...
      return {
        toolResult: {
          content: [
            { type: "text", text: JSON.stringify(paymentGateways, null, 2) },
          ],
        },
      };
    } catch (error: any) {
      ...
      return {
        toolResult: {
          ...
          content: [
            {
              type: "text",
              text: `Error listing payment gateways: ${errorMessage}`,
            },
          ],
        },
      };
  ...
  get_payment_gateway: async (params: GetPaymentGatewayParams) => {
    try {
      const response = await makeWooCommerceRequest(
        "GET",
        `payment_gateways/${params.id}`,
      );
      const paymentGateway: WooCommercePaymentGateway = response;
      return {
        toolResult: {
          content: [
            { type: "text", text: JSON.stringify(paymentGateway, null, 2) },
          ],
        },
      };
    } catch (error: any) {
      ...
      return {
        toolResult: {
          ...
          content: [
            {
              type: "text",
              text: `Error getting payment gateway: ${errorMessage}`,
            },
          ],
        },
      };
      ...
      return {
        toolResult: {
          ...
          content: [
            {
              type: "text",
              text: `Error updating payment gateway: ${errorMessage}`,
            },
          ],
        },
      };
